...
class LogisticRegression:
    ...
    def __init__(self, dimension, num_classes):
        ...
        probs = T.nnet.softmax(T.dot(X, self.W) + self.b)   # Probs that target = some class
        prediction = T.argmax(probs, axis=1)                # The prediction thresholded
        cost = -T.mean(T.log(probs)[T.arange(y.shape[0]), y])
