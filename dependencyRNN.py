...
class DependencyRNN:
    ...
    def __init__(self, d, V, r, answer_idxs, embeddings=None, seed=0):
        ...
        def normalized_tanh(x):
            ...
            tan_h = T.tanh(x)
            return tan_h/T.sqrt(T.sum(T.dot(tan_h, tan_h)))
        ...
        def recurrence(n, hidden_states, hidden_sums, cost, x, r, p, wrong_ans, corr_ans):
            ...
            h_sum = hidden_sums[p[n]] + T.dot(r[n], h_n)

            c_term = T.dot(corr_ans, h_n) # x_c \dot h_n
            z_terms = T.dot(wrong_ans, h_n)
            new_cost = cost + T.sum(\
                T.maximum(0., 1. + z_terms-c_term)\
            )
            return [T.set_subtensor(hidden_states[n], h_n),
                    T.set_subtensor(hidden_sums[p[n]], h_sum),
                    new_cost]
